...
| 항목 | 지금 결정 필요? | 트리거 |
|---|---|---|
| 1. 청크 미분산 | **예 — 실측 확인됨** | 부하 테스트(`load-test-report.md`)에서 처리량이 목표(20,000 msg/sec) 대비 약 179배 부족한 것을 실측함 |
| 2. Read Model 동기화 레이스 | **예 — 실측 확인됨** | 장애 주입 테스트(`chaos-test-report.md` §1.1)에서 실제 알림 영구 유실로 이어지는 것을 관측함 |
| 3. Relay 무한 재시도 | 아니오 (정상 동작 확인됨) | 장애 주입 테스트에서 재시도 폭주 없이 결국 전량 발행됨을 확인(`chaos-test-report.md` §1) |
| 4. Relay 동기 블로킹 발행 | **예 — 실측 확인됨** | 부하 테스트가 만든 백로그 처리 중 개별 발행이 최대 120초까지 블로킹되는 것을 관측(`chaos-test-report.md` §1.1) |
| 5. Consumer group 공유 / 스케줄러 공유 | **예 — 인스턴스 스케일아웃 전에 + 단일 인스턴스에서도 실측된 지연 문제** | 장애 주입 테스트에서 한 Relay의 백로그가 다른 Relay를 지연시키는 것을 실측(`chaos-test-report.md` §1.1) |
